...
Usage: mdbench [options] <PATH>
  ...
  where options are:
	...
	--sync               : flush and sync files
	--direct             : use O_DIRECT for file creation
	--chunk              : size of a chunk for single write operation, default 1024
    -h, --help           : help message
...
def make_files(root, dir_count, file_count, size = 0, random_data = False, chunk = 1024, sync = False, direct = False):

	for j in range(file_count):
		if dir_count > 0:
			for i in range(dir_count):
				mkfile(gen_file( gen_dir(root, i), j ), size, chunk, random_data = random_data, sync = sync, direct = direct)
		else:
			mkfile(gen_file(root, j), size, chunk, random_data = random_data)
...
def mkfile(fname, size, chunk = 65536, sync = False, random_data = False, direct = False) :
	...
	if direct:
			f = os.open('file', os.O_CREAT | os.O_DIRECT | os.O_RDWR | os.O_TRUNC)
			while remaining > 0:
...
def main():
	...
	try:
		options, remainder = getopt.gnu_getopt(sys.argv[1:], 'f:d:s:c:nher', \
					 ['files=','dirs=','size=','chunk=','no-clean','no-container','extended-checks','csv-file=','random-data','sync','direct','help'])
	...
	make_files(root, dir_count, file_count, file_size, random_data=random_data, sync=file_sync, direct=file_direct, chunk=file_chunk)
